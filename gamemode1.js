...
class TowerDefenseGame {
    ...
    async startGameAfterVideo() {
        ...
        popupManager.addMessage("The year is 2042. Earth is a distant memory.");
        popupManager.addMessage("We fled our dying world, only to find ourselves in a new kind of hell.");
        popupManager.addMessage("This planet, Terravivaria, is hostile. We are not welcome here.");
        popupManager.addMessage("The native lifeforms, the 'Ordinaries' as we call them, are relentless.");
        popupManager.addMessage("They are destroying our colonies, one by one.");
        popupManager.addMessage("We must defend ourselves. We must survive.");
        popupManager.addMessage("Build towers to defend your colony. Protect the last of humanity.");
        popupManager.addMessage("Use the WASD keys to move, and the mouse to look around.");
        popupManager.addMessage("Click to place towers. Select different towers from the menu on the left.");
        popupManager.addMessage("Good luck, Commander. You'll need it.");
    ...
    async createScene() {
        ...
        this.alienBase = new AlienBase(this.scene);
    ...
    async spawnEnemy() {
        ...
        if (isBuilder) {
            // Assign a random patrol path
            const patrolPointCount = Math.floor(Math.random() * 3) + 2; // 2 to 4 patrol points
            for (let i = 0; i < patrolPointCount; i++) {
                patrolPath.push(this.patrolWaypoints[Math.floor(Math.random() * this.patrolWaypoints.length)]);
            }
            patrolPath.push(this.alienBase.mesh.position); // Final destination is the alien base
            endPoint = patrolPath[0];
        } else {
            if (!this.targetColony) return; // Don't spawn until a colony exists
            endPoint = this.targetColony.mesh.position;
        }

        this.createEnhancedPath(startPoint, endPoint);
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            if (enemy.pathIndex < enemy.path.length - 1) {
                ...
                if (enemy.pathProgress >= 1) {
                    ...
                    if (enemy.isBuilder && enemy.patrolIndex < enemy.patrolPath.length -1) {
                        enemy.patrolIndex++;
                        this.createEnhancedPath(enemy.mesh.position, enemy.patrolPath[enemy.patrolIndex]);
                ...
                if (enemy.pathIndex < enemy.path.length - 1) {
                    enemy.mesh.position = BABYLON.Vector3.Lerp(
                        ...
                        enemy.path[enemy.pathIndex + 1],
                        enemy.pathProgress
                    );
                    
                    // Face movement direction
                    const direction = enemy.path[enemy.pathIndex + 1].subtract(enemy.path[enemy.pathIndex]);
                    enemy.mesh.lookAt(enemy.mesh.position.add(direction));
                }
            } else {
                if (enemy.isBuilder) {
                    this.alienBase.addProgress(10);
                    enemy.mesh.dispose();
                    if (enemy.healthBar) enemy.healthBar.dispose();
                    this.enemies.splice(i, 1);
                } else {
